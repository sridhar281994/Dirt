...
def _running_on_android() -> bool:
    """
    python-for-android sets ANDROID_ARGUMENT/ANDROID_PRIVATE at runtime.
    We use that to avoid importing backend-only dependencies on Android.
    """
...
if _running_on_android():
    # Android entrypoint (required: a root-level main.py for python-for-android).
    from frontend_app.main import ChatApp

    ChatApp().run()
else:
    # Backend entrypoint (uvicorn imports `app` from here).
    ...
    from routers.ws_signaling import router as ws_router
